...
class PlugSaaSClient {
  ...
  async request(method, endpoint, data = null) {
    ...
    // Add API key if available (supports both Bearer and X-API-KEY headers)
    if (this.apiKey) {
    ...
    try {
      console.log(`PlugSaaS API Request: ${method} ${url}`);
      ...
      try {
        ...
        // If response is HTML (like a 404 page), show helpful error
        if (responseText.includes('<!doctype') || responseText.includes('<html')) {
          console.error(`PlugSaaS API returned HTML instead of JSON. URL may be incorrect: ${url}`);
          throw new Error(`API returned HTML page instead of JSON. Check PLUGSAAS_BASE_URL. Status: ${response.status}`);
        }
        throw new Error(`Invalid JSON response: ${responseText.substring(0, 200)}`);
      }

      if (!response.ok) {
        console.error('PlugSaaS API Error Response:', JSON.stringify(responseData, null, 2));
        console.error('PlugSaaS API Response Status:', response.status);
        console.error('PlugSaaS API Response Text:', responseText);
        throw new Error(responseData.message || responseData.error || responseData.detail || JSON.stringify(responseData) || `API Error: ${response.status}`);
      }
